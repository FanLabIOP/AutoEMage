...
#my $gain_dm4 = `ls ${EM_dir}*dm4 | tail -1`; #展示所有 dm4 文件，为了找 gain 文件
#chomp($gain_dm4); #去掉换行
if (-e "${EM_dir}reference/gain.mrc") 
{
	system "cp ${EM_dir}reference/gain.mrc ${USB_dir}gain_8bit.mrc"; #复制 gain 文件
	print "复制 gain.mrc\n";
}
elsif (-e "${EM_dir}reference/K3-18480084 Gain Ref. x1.m1.kv[300].dm4") 
{
	system "dm2mrc '${EM_dir}reference/K3-18480084 Gain Ref. x1.m1.kv[300].dm4' ${USB_dir}gain_8bit.mrc";
	print "将 dm4 文件转换为 mrc 文件\n";
}
else    {print "gain 文件不存在！"; exit;}
sleep(10);
#-----gain process end-----

my $file_postfix = ".tiff"; #初始照片文件的后缀
...
while ($square_num_old < $square_num) 
{
	for (my $i = 0; $i < $grids_num_new; $i++) 
	{
		...
		my @new_stacks = glob "${new_folder}/FoilHole_*_fractions$file_postfix"; 
		...
		while ($stacks_num_new <= 0) 
		{
			...
			@new_stacks = glob "${new_folder}/FoilHole_*_fractions$file_postfix"; 
		...
		for (my $k = 0; $k < $stacks_num_new; $k++) 
		{
    		...
    		{
    			sleep(20);
    			print "GPU is in use! Wait for 20 seconds.\n";
			...
			{
				print "Auto_mv_corr.pl -mode $mode -EM_dir $EM_dir -USB_dir $USB_dir -job $job_name -raw_num $count -rename_file $new_file -disk_space $diskspace -psize $psize -total_dose $total_dose -rmi $rmi -acv $acv -file_postfix $file_postfix >> ${INFO_dir}${job_name}_auto_mv_corr.log & \n";
				`Auto_mv_corr.pl -mode $mode -EM_dir $EM_dir -USB_dir $USB_dir -INFO_dir $INFO_dir -job $job_name -raw_num $count -rename_file $new_file -disk_space $diskspace -psize $psize -total_dose $total_dose -rmi $rmi -acv $acv -file_postfix $file_postfix >> ${INFO_dir}${job_name}_auto_mv_corr.log &`;
			}
			else 
			{
				print "Auto_mv_corr.pl -mode $mode -EM_dir $EM_dir -USB_dir $USB_dir -job $job_name -raw_num $count -rename_file $new_file -disk_space $diskspace -psize $psize -total_dose $total_dose -rmi $rmi -acv $acv -file_postfix $file_postfix >> ${INFO_dir}${job_name}_auto_mv_corr.log\n";
